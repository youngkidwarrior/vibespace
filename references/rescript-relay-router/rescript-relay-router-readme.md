...
Check in the generated ReScript router source files. In the default layout,
that means committing `src/routes/__generated__/*.res` and
`src/routes/__generated__/*.resi` alongside `src/routes/routes.json` and route
renderer files.

Do not check in compiled ReScript JavaScript output such as `*.res.js` or
source maps. Those are build artifacts and should stay ignored.

### Cleaning up

Do not hand-edit generated router files. Change `routes.json` or route
renderers, then regenerate with:
...
CI should verify that checked-in generated router files are fresh. Run:

```sh
yarn router
```

Then fail the build if the command leaves a git diff in route declarations,
route renderers, or generated router source files. For example:

```sh
git diff --exit-code -- src/routes
```
