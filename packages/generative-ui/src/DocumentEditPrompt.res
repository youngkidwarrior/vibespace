...
let composeProfileEditPrompt = (input: profileEditPromptInput) => {
  ...
  [
    ...
    "HTML/CSS defaults: use standard semantic HTML, safe static inline SVG for decorative/vector UI, class-based styling, responsive flex/grid layouts, CSS variables, transforms, transitions, and keyframes. CSS must parse as plain browser CSS: use only /* ... */ for comments, never slash-only comments like / note /, never leave a selector ending in a combinator such as .card > { ... }, and always prefix class selectors with a dot. Do not write rules for the html or body selectors; the editor host owns those so the iframe keeps its own scroll container on mobile. Scope every visual rule to .profile-... classes or other descendant selectors of <main>. Do not put aria-label on profile content elements like <section>, <div>, <article>, <main>, <h1>-<h6>, <p>, <span>, <ul>, <li>, <figure>, or any inline SVG element. Use visible text, captions, data-vibespace-name, data-vibespace-description, and data-vibespace-alt instead. aria-label is permitted only on <a>, <area>, <button>, <img>, <input>, <select>, <summary>, <textarea>, or on an element that carries an explicit role of button, checkbox, combobox, dialog, group, img, link, listbox, menuitem, option, progressbar, radio, region, searchbox, slider, spinbutton, switch, tab, tabpanel, or textbox. Examples: <section aria-label=\"...\"> is forbidden; <button aria-label=\"...\"> and <div role=\"region\" aria-label=\"...\"> are allowed. Keep visible profile copy natural for non-developers; do not mention implementation terms.",
    "Safe SVG contract: SVG may be authored inline for decorative shapes, icons, stickers, badges, dividers, and visual texture. Use only static SVG geometry, gradients, clip paths, titles, and descriptions. Do not use SVG scripts, foreignObject, image, use, animation tags, linked resources, event handlers, inline styles, or remote SVG files.",
    ...
    "",
    "<preflight_check>",
    "Before returning, check that html is a balanced <template> fragment with one <main> root, double-quoted attributes, meaningful anchors/metadata, no forbidden constructs, no invalid aria-label attributes, and no raw remote resources. Check that css is broadly supported, class-based, contains no remote loads, uses valid /* ... */ comments, and has no incomplete selectors.",
    "</preflight_check>",
    "",
    "<response_contract>",
